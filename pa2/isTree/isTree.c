...
bool isTreeDFS (
    ...
    /* ... */
    //printf(visited[current] ? "true" : "false");
    if(visited[current]) {
        //printf(" returned fasle ");
        return false;
    }
    ...
    while (neighbor) {
        if (neighbor->graphNode!=parent) {
            ...
            /* ... */
            //printf(" %ld %ld %ld\n", parent, current, neighbor->graphNode);
...
int main ( int argc, char* argv[] ) {
    ...
    /* ... */
    bool isTree = isTreeDFS(graphNodeCount, adjacencyList, visited, adjacencyList->graphNode, adjacencyList->graphNode);
