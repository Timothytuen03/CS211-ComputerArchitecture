...
void accessData (
    ...
    cache_line_t* newEnqueue = (cache_line_t*)malloc(sizeof(cache_line_t));
    newEnqueue->tag = tag;
    newEnqueue->next_cache_line = NULL;
    if(cache->occupancy == 0) {
        cache->front = newEnqueue;
        cache->back = newEnqueue;
    } else {
        cache->back->next_cache_line = newEnqueue;
