...
void accessData (
    ...
    // cache += tag;                    //Move the pointer to the correct address of the cache location we want.
                            //Each block is 4 bits. s+b represents distance away. s = S = 16 bits/blocks/number of sets
    // printf("address: %llx, index: %llx, tag: %llx, cacheTag: %llx ", addr, set, tag, (cache+set)->tag);
